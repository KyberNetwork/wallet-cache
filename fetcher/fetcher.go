...
func (self *InfoData) UpdateTokenAPI(newTokenAPI []ethereum.TokenAPI) {
	self.mu.Lock()
	defer self.mu.Unlock()
	self.TokenAPI = newTokenAPI
}
...
func NewFetcher(kyberENV string) (*Fetcher, error) {
	...
	infoData := InfoData{
		...
		InternalNetworkAbi: INTERNAL_NETWORK_ABI,
	}
	...
	ethereum, err := NewEthereum(infoData.Network, infoData.NetworkAbi, infoData.TradeTopic,
		infoData.Wapper, infoData.WrapperAbi, infoData.AverageBlockTime, infoData.InternalNetwork, infoData.InternalNetworkAbi)
	if err != nil {
		log.Print(err)
		return nil, err
	}
...
// update list reserve for token
func (self *Fetcher) UpdateListReserve() {
	listToken := self.GetListTokenAPI()
	newListToken := []ethereum.TokenAPI{}

	for _, token := range listToken {
		reserveSrcArr := []string{}
		reserveDestArr := []string{}
		isSrc, isDest := true, true
		for i := 0; ; i++ {
			if isSrc == true {
				reserveAddr, err := self.GetReserve("src", token.Address, i)
				if err != nil {
					isSrc = false
				} else {
					reserveSrcArr = append(reserveSrcArr, reserveAddr)
				}
			}
			time.Sleep(3 * time.Second)
			if isDest == true {
				reserveAddr, err := self.GetReserve("dest", token.Address, i)
				if err != nil {
					isDest = false
				} else {
					reserveDestArr = append(reserveDestArr, reserveAddr)
				}
			}
			if isSrc == false && isDest == false {
				newToken := token
				newToken.ReserveSrc = reserveSrcArr
				newToken.ReserveDest = reserveDestArr
				newListToken = append(newListToken, newToken)
				break
			}
			time.Sleep(3 * time.Second)
		}
	}
	self.info.UpdateTokenAPI(newListToken)
}

func (self *Fetcher) GetReserve(typeReserve, tokenAddr string, numInt int) (string, error) {
	var reserverAddr string
	var err error
	var dataAbi string
	if typeReserve == "src" {
		dataAbi, err = self.ethereum.EncodeReservesPerTokenSrcData(tokenAddr, numInt)
	} else {
		dataAbi, err = self.ethereum.EncodeReservesPerTokenDestData(tokenAddr, numInt)
	}
	if err != nil {
		log.Println(err)
		return "", err
	...
	for _, fetIns := range self.fetIns {
		if count > 2 {
			log.Println("cannot find more reserve")
			return "", errors.New("cannot find more reserve")
		}
		count++
		var result string
		result, err = fetIns.EthCall(self.info.InternalNetwork, dataAbi)
		if err != nil {
			continue
		}
		if typeReserve == "src" {
			reserverAddr, err = self.ethereum.ExtractReservesPerTokenSrcData(result)
		} else {
			reserverAddr, err = self.ethereum.ExtractReservesPerTokenDestData(result)
		}
		if err != nil {
			continue
		}
	...
	if err != nil {
		log.Println(err)
		return reserverAddr, err
	}
	return reserverAddr, nil
}
